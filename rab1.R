...
im1 <- plot(x.train, y.train, col = grey(0.2),
     pch = 21, bg = grey(0.2),
     xlab = 'X', ylab = 'Y', 
...
legend('topleft', legend = c('обучение', 'тест', 'f(X)', 'модель'),
       ...
       lty = c(0, 0, 2, 1), lwd = c(1, 1, 2, 2), cex = 1.6)

#  Ответ на теоретический вопрос ################################

# Усиление линейности взаимосвязи повлияло на величину MSE следующим образом: 
# стандартная ошибка на обучающей выборке стала меньше (0.8516178 < 0.8707958); 
# стандартная ошибка на тестовой выборке стала меньше (1.6874485 < 1.7136031). 
# Увеличение числа степеней свободы приводит к тому, что стандартная ошибка 
# на обучающей выборке уменьшается, а на тестовой - увеличивается. 
# В предыдущем примере наблюдали следующую зависимость: увеличение числа 
# степеней свободы приводило к уменьшению стандартной ошибки на обучающей выборке 
# и к её уменьшению на тестовой.